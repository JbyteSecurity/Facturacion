<?php
	...
	$sql_count=mysqli_query($con,"select * from boletas where id_boleta='".$id_boleta."'");
	$count=mysqli_num_rows($sql_count);
	if ($count==0)
	{
	echo "<script>alert('Boleta no encontrada')</script>";
	...
	$fecha_boleta=$rw_boleta['fecha_boleta'];
	$condiciones=$rw_boleta['condiciones'];
	require_once(dirname(__FILE__).'/../html2pdf.class.php');
    // get the HTML
     ob_start();
     include(dirname('__FILE__').'/res/ver_boleta_html.php');
